...
# Fully animated | Floating menu | Responsive
...
:root{--bg:#0a0618;--card:rgba(20,10,45,0.88);--accent:#8B5CF6;--accent2:#A78BFA;--text:#F0E8FF;--dim:#6D4A9E;--mid:#A78BFA;--border:rgba(139,92,246,0.2);--glow:rgba(139,92,246,0.1)}
[data-theme="light"]{--bg:#F5F0FF;--card:rgba(255,255,255,0.88);--accent:#7C3AED;--accent2:#6D28D9;--text:#1A0E30;--dim:#7A5DA6;--mid:#4A2D6E;--border:rgba(124,58,237,0.15);--glow:rgba(124,58,237,0.05)}
...
.card{background:var(--card);border:1px solid var(--border);border-radius:24px;padding:36px 30px;backdrop-filter:blur(30px);box-shadow:0 30px 80px rgba(0,0,0,0.4),0 0 60px var(--glow);position:relative;overflow:hidden;transition:transform 0.4s cubic-bezier(0.16,1,0.3,1),box-shadow 0.4s ease}
.card:hover{transform:translateY(-4px);box-shadow:0 40px 100px rgba(0,0,0,0.5),0 0 80px var(--glow)}
.card::before{content:'';position:absolute;inset:-1px;border-radius:24px;padding:1px;z-index:-1;background:conic-gradient(from var(--ang,0deg),transparent 0%,var(--accent) 12%,transparent 28%,transparent 100%);mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:xor;-webkit-mask-composite:xor;opacity:0.4;animation:spinBorder 6s linear infinite}
...
.brand{display:flex;align-items:center;gap:12px;margin-bottom:24px;animation:fadeIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.1s backwards}
.brand-icon{width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#8B5CF6,#6D28D9);display:flex;align-items:center;justify-content:center;color:#fff;font-size:22px;flex-shrink:0;box-shadow:0 0 40px rgba(139,92,246,0.3);animation:iconPulse 3s ease-in-out infinite}
@keyframes iconPulse{0%,100%{box-shadow:0 0 40px rgba(139,92,246,0.3)}50%{box-shadow:0 0 60px rgba(139,92,246,0.5)}}
.brand-name{font-size:18px;font-weight:800;color:var(--text);background:linear-gradient(135deg,var(--text),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.brand-sub{font-size:11px;color:var(--dim);margin-top:2px}
h1{font-size:20px;font-weight:800;color:var(--text);margin-bottom:4px;animation:fadeIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.2s backwards}
.sub{font-size:12.5px;color:var(--mid);margin-bottom:22px;line-height:1.7;animation:fadeIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.3s backwards}
@keyframes fadeIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
.hint{display:flex;align-items:center;gap:10px;background:rgba(139,92,246,0.05);border:1px dashed var(--border);border-radius:10px;padding:9px 14px;margin-bottom:18px;animation:fadeIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.4s backwards;transition:border-color 0.3s,background 0.3s}
.hint:hover{border-color:var(--accent);background:rgba(139,92,246,0.08)}
.hint-label{font-size:11px;color:var(--dim);flex:1}
.hint-val{font-family:ui-monospace;font-size:13px;font-weight:700;color:var(--accent);background:rgba(139,92,246,0.12);border:1px solid rgba(139,92,246,0.2);padding:3px 10px;border-radius:6px;cursor:pointer;transition:all 0.3s cubic-bezier(0.16,1,0.3,1)}
...
.field{margin-bottom:16px;animation:fadeIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.5s backwards}
.field label{display:block;font-size:10.5px;font-weight:700;color:var(--dim);margin-bottom:6px;text-transform:uppercase;letter-spacing:0.06em}
.inp-wrap{position:relative}
input[type=password]{width:100%;padding:12px 42px 12px 14px;border-radius:12px;border:1.5px solid var(--border);background:rgba(0,0,0,0.2);color:var(--text);font-family:inherit;font-size:14px;outline:none;transition:all 0.3s cubic-bezier(0.16,1,0.3,1)}
...
.ic-lock{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--dim);font-size:17px;pointer-events:none;transition:0.3s}
...
.footer{margin-top:20px;padding-top:16px;border-top:1px solid var(--border);display:flex;flex-direction:column;align-items:center;gap:4px;font-size:10.5px;color:var(--dim);animation:fadeIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.7s backwards}
...
@media(max-width:420px){.card{padding:28px 20px;border-radius:18px}.hint{flex-wrap:wrap}}
...
<div class="wrap">
<div class="card">
<div class="brand"><div class="brand-icon"><i class="ti ti-brand-azure"></i></div><div><div class="brand-name">Purple-Panel</div><div class="brand-sub">v1.1 · انیمیشنی</div></div></div>
<h1>ورود به پنل</h1>
...
applyTheme(isDark);
// ── پارتکل‌ها ──
(function(){const box=document.getElementById('particles');for(let i=0;i<20;i++){const p=document.createElement('div');p.className='particle';const s=2+Math.random()*4;p.style.width=s+'px';p.style.height=s+'px';p.style.left=Math.random()*100+'vw';p.style.setProperty('--drift',(Math.random()*80-40)+'px');p.style.animationDuration=(12+Math.random()*18)+'s';p.style.animationDelay=(Math.random()*15)+'s';box.appendChild(p)}})();
// ── ریپل ──
...
:root{
  ...
  --card:#1a0d30;--card-b:rgba(139,92,246,0.12);--card-bh:rgba(139,92,246,0.3);
...
[data-theme="light"]{
  ...
  --card:#FFFFFF;--card-b:rgba(139,92,246,0.14);--card-bh:rgba(139,92,246,0.3);
...
.sidebar .logo-icon{width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#8B5CF6,#6D28D9);display:flex;align-items:center;justify-content:center;color:#fff;font-size:18px;box-shadow:0 0 20px rgba(139,92,246,0.2);animation:logoPulse 3s ease-in-out infinite}
...
.sb-item{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;color:var(--t3);font-size:12.5px;cursor:pointer;transition:all 0.25s cubic-bezier(0.16,1,0.3,1);border-right:2px solid transparent}
...
.sb-footer button{background:var(--accent-d);border:1px solid var(--card-b);color:var(--t2);border-radius:8px;padding:8px;font-family:inherit;font-size:11.5px;font-weight:600;cursor:pointer;transition:all 0.25s cubic-bezier(0.16,1,0.3,1);display:flex;align-items:center;justify-content:center;gap:6px}
...
.tb-title i{color:var(--accent);font-size:20px;animation:titlePulse 3s ease-in-out infinite}
...
.metric{background:var(--card);border:1px solid var(--card-b);border-radius:var(--radius);padding:16px 16px 12px;transition:all 0.3s cubic-bezier(0.16,1,0.3,1);cursor:default;position:relative;overflow:hidden}
...
.metric .m-icon{width:34px;height:34px;border-radius:8px;background:var(--accent-d);display:flex;align-items:center;justify-content:center;color:var(--accent);font-size:16px;margin-bottom:8px;transition:transform 0.3s ease}
...
.card{background:var(--card);border:1px solid var(--card-b);border-radius:var(--radius);padding:16px 18px;transition:all 0.3s cubic-bezier(0.16,1,0.3,1)}
...
# حذف تابع get_public_page_html که باعث ارور میشد
# اگر نیاز داری، میتونی خودت اضافه کنی
